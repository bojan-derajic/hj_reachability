...
class DoubleIntegrator1D(dynamics.ControlAndDisturbanceAffineDynamics):
    """Class that implements 1D double integrator model."""
    ...
    def open_loop_dynamics(self, state, time):
        """Implements open loop dynamics of a 1D double integrator"""
        A = jnp.array([[0.0, 1.0], [0.0, 0.0]])
        return A @ state

    def control_jacobian(self, state, time):
        """Calculates control Jacobian of a 1D double integrator"""
        return jnp.array([[0.0], [1.0]])

    def disturbance_jacobian(self, state, time):
        """Calculates disturbance Jacobian of a 1D double integrator"""
        return jnp.array([[0.0], [1.0]])

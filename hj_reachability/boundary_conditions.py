...
def replicate(x: Array, pad_width: int) -> Array:
    """Pads a 1D array `x` by replicating the first and last values."""
    return jnp.pad(x, ((pad_width, pad_width)), "edge")
...
def extrapolate_away_from_zero(x: Array, pad_width: int) -> Array:
    ...
    return jnp.concatenate(
        [
            ...
            x,
            x[-1]
            + jnp.sign(x[-1]) * jnp.abs(x[-1] - x[-2]) * jnp.arange(1, pad_width + 1),
        ]
    )
